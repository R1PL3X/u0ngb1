# CanteenGo on Vercel + Neon

Ứng dụng CanteenGo chạy bằng HTML/CSS/JS tĩnh trên Vercel. Các API nằm trong Vercel Function `api/[...path].js`; dữ liệu nằm ở Neon Postgres, được kết nối từ Vercel Marketplace. Không cần XAMPP, PHP hoặc MySQL khi triển khai.
...
Sau khi mở project, truy cập `demo.html` để xem trước giao diện ở 3 góc nhìn: khách hàng, bếp/KDS và quản trị. Demo không yêu cầu đăng nhập và có nút **Kiểm tra kết nối DB** để gọi `GET /api/db-health` khi project đã cấu hình Neon.

Các trang thật vẫn dùng database như bình thường: `menu.html` đọc món từ `menu_items`, `cart.html` tạo đơn trong `orders` + `order_items`, KDS cập nhật trạng thái đơn và Admin quản lý món/đơn/thống kê.

## 6. Upload GitHub bằng trình duyệt (không cần cài Git)

Có thể tạo repository trên GitHub rồi dùng **Add file → Upload files** để kéo toàn bộ thư mục project vào repository. Sau đó vào Vercel → **Add New Project** → import repository.

Database được quản lý bằng **Neon Postgres** thông qua Vercel Marketplace; Vercel giữ phần deploy/frontend/API, còn Neon cung cấp SQL Editor và dữ liệu PostgreSQL.
